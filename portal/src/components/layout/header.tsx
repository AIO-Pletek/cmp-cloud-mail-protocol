...
export function Header() {
  ...
  const title = pageTitles[pathname] || pathname.split('/').filter(Boolean).pop()?.replace(/-/g, ' ') || 'Dashboard';
        ...
        <nav className="flex items-center space-x-1 text-sm text-gray-500">
          ...
          {segments.map((segment, i) => (
            <span key={i} className="flex items-center space-x-1">
              ...
              <span className={i === segments.length - 1 ? 'text-gray-900 font-medium' : 'hover:text-gray-700'}>
          ...
          {dropdownOpen && (
            <div className="absolute right-0 top-full mt-1 w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm text-danger-600 hover:bg-red-50 w-full"
              >
