...
def _postfix_release(queue_id: str) -> bool:
    try:
        result = subprocess.run(["postsuper", "-H", queue_id], capture_output=True, text=True, timeout=10)
        if result.returncode != 0:
            print(f"[DomainApproval] postsuper -H failed queue_id={queue_id} rc={result.returncode} stdout={result.stdout.strip()} stderr={result.stderr.strip()}")
            return False
    ...
    for attempt in range(6):
        ...
        if qid:
            ...
            try:
                ...
                await c.close()
            print(f"[DomainApproval] Resolved queue_id={qid} for approval {approval_id[:8]}")
            return
        await asyncio.sleep(2)
    print(f"[DomainApproval] Could not resolve queue_id for approval {approval_id[:8]}")
