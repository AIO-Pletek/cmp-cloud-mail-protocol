...
async def get_queue_stats() -> dict:
    ...
    items = _parse_postqueue_text(output)
    total = len(items)
    ...
    for line in output.split("\n"):
        if len(line) > 10 and line[0].isalnum() and " " in line[:20]:
            ...
            if len(parts) >= 7:
                try:
                    ts = datetime.strptime(" ".join(parts[2:7]), "%a %b %d %H:%M:%S").replace(year=datetime.now().year)
                    delta = int((datetime.now() - ts).total_seconds() / 60)
                    if oldest_age is None or delta > oldest_age:
                        oldest_age = delta
                except Exception:
                    pass
    return {"total": total, "active": active, "hold": hold, "deferred": deferred, "oldest_age": oldest_age}
