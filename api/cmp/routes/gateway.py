...
async def require_admin_tenant(tenant):
    """Ensure only admin can access gateway settings."""
...
@router.get("/config")
async def get_config(tenant=Depends(require_admin_tenant)):
    """Get full enterprise gateway configuration."""
    return await get_relay_auth_config()


@router.put("/config")
async def update_config(data: dict, tenant=Depends(require_admin_tenant)):
...
@router.get("/api-keys")
async def list_keys(tenant=Depends(require_admin_tenant)):
...
async def create_key(data: ApiKeyCreate, tenant=Depends(require_admin_tenant)):
...
async def revoke_key(key_id: str, tenant=Depends(require_admin_tenant)):
    """Revoke an API key."""
    return await revoke_api_key(key_id)


@router.post("/api-keys/verify")
async def verify_key(key: str, client_ip: str = "", tenant=Depends(require_admin_tenant)):
...
async def list_hosts(tenant=Depends(require_admin_tenant)):
...
async def add_host(data: TrustedHostAdd, tenant=Depends(require_admin_tenant)):
...
async def remove_host(address: str, tenant=Depends(require_admin_tenant)):
...
async def get_limits(tenant=Depends(require_admin_tenant)):
    """Get rate limit configuration."""
    return await get_rate_limits()


@router.put("/rate-limits")
async def update_limits(data: RateLimitUpdate, tenant=Depends(require_admin_tenant)):
    """Update rate limits."""
