...
# ---------------------------------------------------------------------------
# Fix 3: Per-domain approval_required toggle
# GET  /api/v1/policy/domain/{domain_id}/approval
# PUT  /api/v1/policy/domain/{domain_id}/approval
# ---------------------------------------------------------------------------
from cmp.policy.domain_policy_store import (
    get_domain_approval_required as _get_domain_approval_required,
    set_domain_approval_required as _set_domain_approval_required,
)


@router.get("/domain/{domain_id}/approval")
async def get_domain_approval(domain_id: str, user=Depends(get_current_user)):
    """Get the per-domain approval_required flag."""
    result = await _get_domain_approval_required(domain_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Domain not found")
    return result


@router.put("/domain/{domain_id}/approval")
async def set_domain_approval(domain_id: str, body: ToggleBody, user=Depends(get_current_user)):
    """Enable or disable approval requirement for a specific domain."""
    result = await _set_domain_approval_required(domain_id, body.enabled)
    if result is None:
        raise HTTPException(status_code=404, detail="Domain not found")
    return result
...
# ---------------------------------------------------------------------------
from cmp.policy.domain_policy_store import (
    get_domain_attachment_password_required as _get_domain_ap_required,
    set_domain_attachment_password_required as _set_domain_ap_required,
)
from cmp.services.attachment_password_map import sync_attachment_password_map


@router.get("/domain/{domain_id}/attachment-password")
async def get_domain_attachment_password(domain_id: str, user=Depends(get_current_user)):
    """Get the per-domain attachment_password_required flag."""
    result = await _get_domain_ap_required(domain_id)
    if result is None:
        raise HTTPException(status_code=404, detail="Domain not found")
    return result


@router.put("/domain/{domain_id}/attachment-password")
async def set_domain_attachment_password(domain_id: str, body: ToggleBody, user=Depends(get_current_user)):
    """Enable or disable the attachment password requirement for a domain."""
    result = await _set_domain_ap_required(domain_id, body.enabled)
    if result is None:
        raise HTTPException(status_code=404, detail="Domain not found")
    try:
        await sync_attachment_password_map()
    except Exception:
        pass  # map sync failure must not break the API; rspamd keeps previous map
    return result
